...
def build_language_questions(language):
    """Build 50 language-specific MCQs with four real answer choices."""
    facts = {
        "C": [
            ("Which function is the usual entry point of a C program?", "main()", ["start()", "run()", "begin()"]),
            ("Which header provides printf() and scanf() in C?", "stdio.h", ["stdlib.h", "string.h", "math.h"]),
            ("Which symbol ends a normal C statement?", ";", [":", ".", ","]),
            ("Which operator gives the address of a variable in C?", "&", ["*", "%", "#"]),
            ("Which operator dereferences a pointer in C?", "*", ["&", "@", "$"]),
            ("Which keyword declares a constant value in C?", "const", ["fixed", "constant", "readonly"]),
            ("Which loop is guaranteed to execute its body at least once in C?", "do-while", ["for", "while", "foreach"]),
            ("Which format specifier is commonly used for an int with printf()?", "%d", ["%f", "%c", "%s"]),
            ("Which data type stores a single character in C?", "char", ["string", "character", "text"]),
            ("Which function compares two C strings?", "strcmp()", ["strcompare()", "compare()", "equalstr()"]),
        ],
        "C++": [
            ("Which function is the usual entry point of a C++ program?", "main()", ["start()", "run()", "begin()"]),
            ("Which stream is commonly used for output in C++?", "cout", ["print", "output", "writeout"]),
            ("Which header provides cout?", "iostream", ["stdio.h", "ostream.h", "streamio"]),
            ("Which keyword creates an object dynamically in C++?", "new", ["create", "malloc", "object"]),
            ("Which operator is used with cout for insertion?", "<<", [">>", "::", "->"]),
            ("Which feature allows the same function name with different parameters?", "Function overloading", ["Function hiding", "Function copying", "Function nesting"]),
            ("Which concept bundles data and methods into one unit?", "Encapsulation", ["Recursion", "Compilation", "Iteration"]),
            ("Which keyword is used to inherit a class?", "class with inheritance syntax", ["inherits", "extends", "parent"]),
            ("Which container stores a dynamic sequence in the C++ STL?", "vector", ["arraylist", "sequence", "listbox"]),
            ("Which symbol accesses a member through an object pointer?", "->", [".", "::", "&"]),
        ],
        "Java": [
            ("Which method is the usual entry point of a Java application?", "public static void main(String[] args)", ["start()", "run()", "main() only"]),
            ("Which keyword creates a Java object?", "new", ["create", "object", "make"]),
            ("Which keyword is used to inherit a class in Java?", "extends", ["inherits", "implements", "superclass"]),
            ("Which keyword is used when a class implements an interface?", "implements", ["extends", "interface", "inherits"]),
            ("Which package contains ArrayList?", "java.util", ["java.io", "java.lang", "java.net"]),
            ("Which type stores true or false in Java?", "boolean", ["bool", "logical", "bit"]),
            ("Which keyword prevents a variable from being reassigned?", "final", ["constant", "fixed", "readonly"]),
            ("Which mechanism handles runtime exceptions?", "try-catch", ["if-else only", "switch-case", "check-error"]),
            ("Which class is the root of the Java class hierarchy?", "Object", ["Class", "Root", "Base"]),
            ("Which collection does not allow duplicate elements?", "Set", ["List", "Array", "Sequence"]),
        ],
        "Python": [
            ("Which keyword defines a function in Python?", "def", ["function", "func", "define"]),
            ("Which function displays output in Python?", "print()", ["echo()", "display()", "write()"]),
            ("Which symbol starts a Python comment?", "#", ["//", "/*", "--"]),
            ("Which data type stores an ordered mutable collection?", "list", ["tuple", "set", "dict"]),
            ("Which data type stores key-value pairs?", "dict", ["list", "tuple", "set"]),
            ("Which keyword handles exceptions in Python?", "try", ["catch", "exceptonly", "handle"]),
            ("Which keyword creates a class in Python?", "class", ["struct", "object", "type"]),
            ("Which operator performs exponentiation in Python?", "**", ["^", "^^", "exp"]),
            ("Which function returns the number of items in a Python collection?", "len()", ["size()", "countall()", "length()"]),
            ("Which keyword exits a loop immediately?", "break", ["exitloop", "stop", "returnloop"]),
        ],
        "JavaScript": [
            ("Which keyword declares a block-scoped variable that can be reassigned?", "let", ["varonly", "change", "define"]),
            ("Which keyword declares a block-scoped constant?", "const", ["constant", "fixed", "static"]),
            ("Which method writes a message to the browser console?", "console.log()", ["print()", "console.write()", "log.console()"]),
            ("Which symbol is commonly used for strict equality?", "===", ["==", "=", "!== only"]),
            ("Which structure stores an ordered collection in JavaScript?", "Array", ["ListOnly", "Tuple", "Sequence"]),
            ("Which function converts JSON text into a JavaScript value?", "JSON.parse()", ["JSON.read()", "JSON.convert()", "parseJSONText()"]),
            ("Which method adds an item to the end of an array?", "push()", ["append()", "add()", "insertEnd()"]),
            ("Which keyword creates a class in modern JavaScript?", "class", ["object", "type", "prototype-class"]),
            ("Which operator is used for logical AND?", "&&", ["||", "and", "&"]),
            ("Which keyword is commonly used to define an asynchronous function?", "async", ["awaitonly", "promise", "defer"]),
        ],
        "HTML/CSS": [
            ("Which HTML element creates a hyperlink?", "a", ["link", "href", "url"]),
            ("Which HTML element represents the main heading?", "h1", ["heading", "head", "title1"]),
            ("Which HTML attribute provides alternative text for an image?", "alt", ["src", "text", "caption"]),
            ("Which CSS property changes text color?", "color", ["font-color", "text-color", "foreground"]),
            ("Which CSS property changes the background color?", "background-color", ["bg-color", "background", "color-bg"]),
            ("Which CSS property controls space inside an element's border?", "padding", ["margin", "spacing", "inside-gap"]),
            ("Which CSS property controls space outside an element's border?", "margin", ["padding", "outside-gap", "border-space"]),
            ("Which CSS layout system is designed for one-dimensional layouts?", "Flexbox", ["Grid", "FloatBox", "TableLayout"]),
            ("Which CSS layout system is designed for two-dimensional layouts?", "Grid", ["Flexbox", "Inline", "PositionBox"]),
            ("Which HTML element is used to create a form?", "form", ["input", "fieldset-only", "submit"]),
        ],
    }

    base = facts[language]
    questions = []
    # Five different question phrasings for each of the ten core concepts.
    prefixes = [
        "Choose the correct answer: ",
        "For a programmer using this technology, which is correct? ",
        "Which option is technically correct? ",
        "A developer is working with this technology. What is correct? ",
        "Select the correct programming answer: ",
    ]
    for round_no in range(5):
        for fact_no, (question, correct, wrongs) in enumerate(base):
            questions.append({
                "question": prefixes[round_no] + question,
                "correct": correct,
                "wrong": list(wrongs),
            })
    return questions[:50]
