...
def initialize_database():
    ...
    admin = cur.execute(
        ...
        (os.environ.get("ADMIN_EMAIL", "admin@capacityconnect.com"),)
    ).fetchone()
    ...
    if not admin:
        ...
        """, (
            ...
            os.environ.get("ADMIN_EMAIL", "admin@capacityconnect.com"),
            generate_password_hash(os.environ.get("ADMIN_PASSWORD", "Admin@123")),
            "admin",
            1
        ))
...
def register():
    ...
    if request.method == "POST":
        ...
        # CAPACITY CONNECT accepts Gmail addresses for verified registration.
        # The OTP verification below proves that the applicant controls the inbox.
        ...
        resend_from_name = os.environ.get("RESEND_FROM_NAME", "CAPACITY CONNECT").strip()

        if not resend_api_key or not resend_from_email:
            flash("Email verification is not configured yet. Please contact the administrator.", "danger")
            return redirect(url_for("register"))

        # Send the OTP through Resend's Email API.
        # This uses Python's standard library, so no extra Resend package is required.
        email_payload = {
            ...
            "html": (
                ...
                f"<h2 style='margin-bottom:8px'>CAPACITY CONNECT</h2>"
                f"<p>Hello {escape(name)},</p>"
                f"<p>Your email verification code is:</p>"
                f"<div style='font-size:32px;font-weight:700;letter-spacing:8px;padding:16px 0'>{otp}</div>"
                f"<p>This OTP is valid for <strong>10 minutes</strong>.</p>"
                f"<p>Do not share this code with anyone.</p>"
                f"<p>Regards,<br>CAPACITY CONNECT</p>"
                f"</div>"
            )
        }
        ...
        try:
            ...
            app.logger.error("Resend API error %s: %s", exc.code, error_body)
            ...
            app.logger.exception("Unable to send registration OTP through Resend: %s", exc)
            flash("We could not send the verification email. Please try again later.", "danger")
            return redirect(url_for("register"))
        ...
        session["pending_registration"] = {
            ...
            "role": role,
            "otp_hash": otp_hash,
            "otp_expires": (datetime.datetime.utcnow() + datetime.timedelta(minutes=10)).timestamp(),
            "otp_attempts": 0
        }

        flash(f"A verification OTP has been sent to {email}.", "success")
...
<p>
Join CAPACITY CONNECT and verify your Gmail to create your account.
</p>
...
placeholder="you@gmail.com" pattern="[A-Za-z0-9._%+-]+@gmail\\.com">
</div>
...
<a href="{{ url_for('login') }}" style="color:#2563eb;font-weight:700">Login</a>
</p>
...
def verify_email():
    ...
    if request.method == "POST":
        ...
        if datetime.datetime.utcnow().timestamp() > pending["otp_expires"]:
            session.pop("pending_registration", None)
            flash("OTP expired. Please register again to receive a new OTP.", "danger")
            return redirect(url_for("register"))
        ...
        if pending["otp_attempts"] > 5:
            session.pop("pending_registration", None)
            flash("Too many incorrect OTP attempts. Please register again.", "danger")
            return redirect(url_for("register"))

        if not check_password_hash(pending["otp_hash"], otp):
            flash("Incorrect OTP. Please check your Gmail and try again.", "danger")
            return redirect(url_for("verify_email"))
        ...
        flash(
            "Email verified. Registration successful. Your account is now waiting for administrator approval.",
            "warning"
        )
        return redirect(url_for("login"))
...
<input class="form-control" name="otp" inputmode="numeric" autocomplete="one-time-code"
maxlength="6" pattern="[0-9]{6}" required placeholder="Enter 6-digit OTP">
</div>
<button class="btn btn-primary" style="width:100%">Verify Email</button>
</form>
<form method="POST" action="{{ url_for('resend_verification') }}" style="margin-top:12px">
<button class="btn btn-secondary" style="width:100%" type="submit">Resend OTP</button>
</form>
<p style="text-align:center;margin-top:20px">
<a href="{{ url_for('register') }}" style="color:#2563eb;font-weight:700">Start again</a>
</p>
</div>
</div>
"""
    return render_page("Verify Email", body, email=pending["email"])
...
def resend_verification():
    ...
    # Generate a fresh OTP and restart the 10-minute validity period.
    otp = f"{secrets.randbelow(1000000):06d}"
    pending["otp_hash"] = generate_password_hash(otp)
    pending["otp_expires"] = (datetime.datetime.utcnow() + datetime.timedelta(minutes=10)).timestamp()
    ...
    email_payload = {
        "from": f"{resend_from_name} <{resend_from_email}>",
        "to": [pending["email"]],
        "subject": "CAPACITY CONNECT - New Verification OTP",
        "html": (
            f"<div style='font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:24px'>"
            f"<h2>CAPACITY CONNECT</h2>"
            f"<p>Hello {escape(pending['name'])},</p>"
            f"<p>Your new email verification code is:</p>"
            f"<div style='font-size:32px;font-weight:700;letter-spacing:8px;padding:16px 0'>{otp}</div>"
            f"<p>This new OTP is valid for <strong>10 minutes</strong>.</p>"
            f"<p>Do not share this code with anyone.</p>"
            f"</div>"
        )
    }
    ...
    try:
        ...
        error_body = exc.read().decode("utf-8", errors="replace")
        app.logger.error("Resend API error %s: %s", exc.code, error_body)
        flash("We could not resend the verification email. Please try again later.", "danger")
        return redirect(url_for("verify_email"))
    except (urllib_error.URLError, TimeoutError, RuntimeError, OSError) as exc:
        app.logger.exception("Unable to resend registration OTP through Resend: %s", exc)
        flash("We could not resend the verification email. Please try again later.", "danger")
        return redirect(url_for("verify_email"))
...
if __name__ == "__main__":
    ...
    print()
    print("ADMIN LOGIN")
    print("Email    :", os.environ.get("ADMIN_EMAIL", "admin@capacityconnect.com"))
