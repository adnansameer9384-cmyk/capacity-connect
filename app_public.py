    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if role != "admin" and not user["approved"]:
                if role == "trainer":
                    if not user["assessment_allowed"]:
                        flash(
                            "Your trainer application is waiting for administrator approval.",
                            "warning"
                        )
                        return redirect(url_for("login"))
...
def login():
    ...
    if request.method == "POST":
        ...
        # A newly registered trainer cannot enter the portal or assessment
        # until an administrator explicitly allows the application to proceed.
        if user["role"] == "trainer" and not user["approved"] and not user["assessment_allowed"]:
            flash(
                "Your trainer application is waiting for administrator approval before you can take the assessment.",
                "warning"
            )
            return redirect(url_for("login"))
...
def trainer_language():
    ...
    if not user["approved"] and not user["assessment_allowed"]:
        flash(
            "Your trainer application must be approved by the administrator before you can take the assessment.",
            "warning"
        )
        return redirect(url_for("login"))
...
def trainer_assessment():
    ...
    if request.method == "POST":
        ...
        conn.execute(
            """
            UPDATE users
            SET approved=?, assessment_allowed=?
...
{% elif user["role"] == "trainer" and user["assessment_allowed"] %}
...
{% if not user["approved"] and user["role"] == "trainer" and not user["assessment_allowed"] %}
...
{% elif not user["approved"] and user["role"] != "trainer" %}
...
{% elif user["role"] == "trainer" and user["assessment_allowed"] and not user["approved"] %}
...
def approve_user(user_id):
    ...
    target = conn.execute(
        "SELECT role, approved, assessment_allowed FROM users WHERE id=?",
        (user_id,)
    ).fetchone()
    ...
    if target["role"] == "trainer":
        conn.execute("""
            UPDATE users
            SET assessment_allowed=1
            WHERE id=? AND role='trainer'
        """, (user_id,))
        message = "Trainer application approved for assessment."
    else:
        conn.execute("""
            UPDATE users
            SET approved=1
