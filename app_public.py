...
def initialize_database():
    ...
    CREATE TABLE IF NOT EXISTS users (
        ...
        assessment_allowed INTEGER DEFAULT 0,
    ...
    if "assessment_allowed" not in user_columns:
        cur.execute("ALTER TABLE users ADD COLUMN assessment_allowed INTEGER DEFAULT 0")
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if role != "admin" and not user["approved"]:
                flash("Your account is waiting for administrator approval. You cannot log in until the administrator approves your application.", "warning")
                return redirect(url_for("login"))
...
def login():
    ...
    if request.method == "POST":
        ...
        # Administrator approval is required before ANY user can log in.
        # This is website-login approval, not assessment approval.
        if not user["approved"]:
            flash(
                "Your account is waiting for administrator approval. You will be able to log in after the administrator approves your application.",
                "warning"
            )
            return redirect(url_for("login"))
        ...
        # Approved trainers select their expert programming language before
...
def trainer_language():
    ...
    if not user["approved"]:
        flash(
            "Your trainer account must be approved by the administrator before you can continue.",
            "warning"
        )
        return redirect(url_for("login"))
...
def trainer_assessment():
    ...
    if request.method == "POST":
        questions = session.get("trainer_assessment_questions", [])
        answer_key = session.get("assessment_answer_key", {})
        if not questions or len(questions) != 50:
            flash("Your assessment session expired. Please start the assessment again.", "warning")
            return redirect(url_for("trainer_language"))
        score = 0
        ...
        if percentage >= 80:
            status = "Passed"
            message = "Congratulations! You scored 80% or above. Your trainer assessment is passed."
        elif percentage >= 50:
            status = "Re-test Required"
            message = "You scored between 50% and 79%. A re-test is required."
        else:
            status = "Failed - Re-test Required"
            message = "You scored below 50%. A re-test is required."
        ...
        # The administrator's login approval remains unchanged by assessment results.
        conn.commit()
        conn.close()
        session.pop("assessment_answer_key", None)
        session.pop("trainer_assessment_questions", None)
        ...
        return render_page(
            ...
            <div class="card" style="max-width:760px;margin:50px auto;text-align:center">
                <div style="font-size:55px">{'🎉' if percentage >= 80 else '📝'}</div>
                ...
                <p>{escape(message)}</p>
                f'<a class="btn btn-primary" href="{url_for("trainer_language")}">Choose Language & Re-test</a>'
    ...
    session["trainer_assessment_questions"] = questions
    ...
    <div class="container">
        ...
        <div class="card" style="margin-bottom:25px">
            ...
            <p class="small"><strong>80–100%:</strong> Passed the trainer assessment.</p>
            <p class="small"><strong>50–79%:</strong> Re-test required.</p>
            <p class="small"><strong>Below 50%:</strong> Failed; re-test required.</p>
            <p class="small"><strong>Important:</strong> Administrator approval controls website login. Assessment results do not approve or block login.</p>
        </div>
...
<div style="display:flex;gap:10px;flex-wrap:wrap">
<a class="btn btn-primary" href="{{ url_for('admin_users') }}">User Management • Approve Users</a>
...
<p>Approve trainee and trainer accounts before they can log in.</p>
...
def approve_user(user_id):
    ...
    # One administrator approval grants permission to log in.
    # Trainer assessment results do not control this login approval.
    conn.execute("UPDATE users SET approved=1 WHERE id=?", (user_id,))
    conn.commit()
    conn.close()

    flash("User approved successfully. They can now log in.", "success")
    return redirect(url_for("admin_users"))
...
# (gunicorn app:app), because __main__ is not executed.
initialize_database()
